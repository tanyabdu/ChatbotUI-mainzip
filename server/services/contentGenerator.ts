...
export async function generateIdeasOnly(input: ContentGenerationInput): Promise<ContentIdea[]> {
  ...
  const systemPrompt = `Ты — маркетолог-стратег, специалист по ПРОГРЕВАМ И ЗАПУСКАМ продуктов. Ты создаёшь контент-план для ПРОГРЕВА к продукту.
...
⚡ КРИТИЧЕСКИ ВАЖНО — СЛЕДУЙ СТРУКТУРЕ ПРОГРЕВА:
${warmupStructure}

Каждый день должен иметь СВОЙ тип контента по структуре!
День 1 ≠ День 7 ≠ День 14 — это РАЗНЫЕ этапы воронки!

ИДЕЯ должна быть РАЗВЁРНУТОЙ (2-3 предложения):
- Что за тема
- Какую боль/возражение закрывает
- В каком формате подать (история, чеклист, разбор и т.д.)

Для каждого дня:
- day: номер
- idea: развёрнутая идея с форматом подачи (2-3 предложения)
- type: тип по структуре (Знакомство/Экспертный/Закрытие возражения/Кейс/Продающий)

Ответь ТОЛЬКО валидным JSON:
{"ideas": [{"day":1,"idea":"развёрнутая идея","type":"тип"}]}`
    : `SEED: ${randomSeed}
...
- В каком формате подать

Для каждого дня:
- day: номер
- idea: развёрнутая идея (2-3 предложения)
- type: тип (Экспертный/Личная история/Вовлекающий/Развлекательный)
