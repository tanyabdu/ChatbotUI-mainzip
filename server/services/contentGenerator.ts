...
ГЛАВНОЕ ПРАВИЛО: Клиент должен СРАЗУ понять, о чём речь. Читает — и всё ясно. Никакой путаницы.

СТИЛЬ ПИСЬМА:
1. Пиши как говоришь — простым, живым языком. Как подруге за чаем.
2. Короткие предложения. Максимум 15-20 слов. Никаких длинных абзацев.
3. Эзотерические термины МОЖНО использовать (Таро, энергии, чакры, ретроград) — но объясняй просто, чтобы понял любой человек.
4. ЗАПРЕЩЕНО:
   - Канцеляризмы: "осуществлять", "производить", "в рамках"
   - Сложные обороты: "в связи с тем, что", "принимая во внимание"
   - Пустые штампы без конкретики
5. ИСПОЛЬЗУЙ:
   - Конкретику: цифры, факты, примеры из жизни
   - Эмоции и вопросы к читателю
   - Простые метафоры
6. Начинай с боли или желания читателя, не с себя.

ПЛОХО (непонятно клиенту):
"Трансмутация энергетических блоков через призму архетипических вибраций откроет портал к вашему истинному Я и активирует потоки изобилия."

ХОРОШО (понятно):
"Чувствуешь, что застряла? Деньги не идут, отношения буксуют. На консультации найдём, что тебя тормозит. Разберём по картам — и составим план, как это изменить."

ЕЩЁ ПРИМЕР:
ПЛОХО: "Ретроградный Меркурий активирует теневые аспекты коммуникативной сферы."
ХОРОШО: "Ретроград Меркурия — это когда всё идёт не по плану: переписки путаются, договорённости срываются. Просто будь внимательнее с документами и не начинай важных дел."`;
...
export async function generateSingleFormat(input: SingleFormatInput): Promise<FormatContent> {
  ...
  const writingRules = `
ГЛАВНОЕ: Клиент читает — и сразу понимает. Никакой путаницы!

СТИЛЬ:
- Пиши как говоришь. Короткими предложениями. Максимум 15 слов.
- Эзотерические термины — можно, но объясняй просто
- НЕЛЬЗЯ: канцеляризмы, сложные обороты, пустые штампы
- МОЖНО: живые эмоции, вопросы, истории, конкретные примеры
- Начинай с боли/желания читателя, не с себя`;
...
Слайд 1 — хук (зацепить листать дальше)
